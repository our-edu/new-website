...
use App\RegistrationApp\Applications\Employee\Transformers\ApplicationServiceTransformer;
use App\RegistrationApp\Applications\Models\ChildApplication;
use Carbon\Carbon;
use League\Fractal\TransformerAbstract;

class ListAnnouncementsTransformer extends TransformerAbstract
{
    ...
    public function includeBranches(Announcement $announcement)
    {
        if (count($announcement->branches)) {
            return $this->collection(
                ...
                new AnnouncementBranchesTransformer(),
                ResourceTypesEnums::BRANCH
            );
        }
    }

    public function includeRoles(Announcement $announcement)
    {
        if (count($announcement->roles)) {
            return $this->collection(
                $announcement->roles,
                new AnnouncementRolesTransformer(),
