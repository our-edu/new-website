...
<main>
    ...
    <div class="welcome text-center">
        ...
        <h1>{{__('payment.bill_number',[],$language)}} : #{{$bill->number}}</h1>
    </div>
    <div class="card">
        <form method="post" action="{{$url}}">
            <div class="card-body">
                ...
                <div class="card-item border-bottom-grey">
                    <div class="card-item_content">{{$bill->payerUser->name}}</div>

                    <div class="card-item_title">{{__('payment.parent_name',[],$language)}}</div>
                </div>
                <div class="card-item border-bottom-grey">
                    <div class="card-item_content">{{$bill->serviceable->name}}</div>

                    <div class="card-item_title">{{__('payment.student_name',[],$language)}}</div>
                </div>

                <div class="card-item border-bottom-grey">
                    <div class="card-item_content">{{$bill->serviceable->parent->user->national_id}}</div>

                    <div class="card-item_title">{{__('payment.parent_national_id',[],$language)}}</div>
                </div>
                <div class="card-item border-bottom-grey">
                    <div class="card-item_content">{{$bill->serviceable->national_id}}</div>

                    <div class="card-item_title">{{__('payment.student_national_id',[],$language)}}</div>
                </div>
                <div class="total">
                    <div class="header">
                        <div class="card-item_content" style="color: #32ba7c">{{$bill->amount_with_vat}}</div>
                        <div class="card-item_title">{{__('payment.full_total_amount',[],$language)}}</div>
                    </div>
                    @foreach($bill->serviceOrder->items as $item)
                        <div class="breakdown">
                            <div class="">{{$item->price}}</div>
                            <div class="">{{($item->serviceGrade()->count() > 0) ? $item->serviceGrade->semester->semester->translate($language)->name : ""}}</div>
                            <div class="">{{$item->service->translate($language)->name}}</div>
                        </div>

                    @endforeach
                    <div class="breakdown">
                        <div class="">{{$bill->vat}}</div>
                        <div class="">{{__('payment.vat',[],$language)}}</div>

                    </div>
                </div>
                <input type="hidden" name="user_id" value="{{$bill->payerUser->uuid}}">
                <input type="hidden" name="amount" value="{{$paid_amount}}">
                @if($device_type)
                    <input type="hidden" name="merchant_extra" value="{{$device_type}}">
                @endif
                @if($redirectPDFLang)
                    <input type="hidden" name="merchant_extra1" value="{{$redirectPDFLang}}">
                @endif
