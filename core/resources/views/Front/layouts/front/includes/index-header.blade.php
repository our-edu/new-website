<div class="homeNav top-header-area" id="sticker">
    <div class="website-container">
        <div class="text-center">
            <div class="main-menu-wrap">
                <!-- logo -->
                <!-- logo -->
                <!-- menu start -->
                <nav class="main-menu">
                    <div class="website-logo">
                        <a href="index.html">
                            <img src="./events-activities/assets/img/logo.png" alt="" />
                        </a>
                    </div>
                    <ul>
                        <li class="current-list-item {{(request()->getRequestUri() == "/")?"active":""}}">
                            <a href="{{route('home')}}">الرئيسية</a>
                        </li>
                        <li><a href="events.html">الفاعليات و الانشطة</a></li>
                        <li><a href="articles.html">المقالات</a></li>
                        <li><a href="{{route('books')}}">الكتب</a></li>
                        <li><a href="images.html">صور </a></li>
                        <li><a href="videos.html">فيديو </a></li>
                        <li><a href="profile.html">الملف الشخصي</a></li>
                        <li><a href="index.html">دراسات بحوث</a></li>
                        <li><a href="contact.html">اتصل بنا</a></li>
                        <li></li>
                    </ul>
                </nav>

                <div class="mobile-menu"></div>
                <!-- menu end -->
            </div>
        </div>
    </div>
</div>
